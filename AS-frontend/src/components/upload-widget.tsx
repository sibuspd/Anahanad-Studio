...
const UploadWidget = ({
  ...
  useEffect(() => {
    // Goal is to always have the onChange prop up to date
    onChangeRef.current = onChange;
  }, [onChange]);

  useEffect(() => {
    // Initialize useEffect on Client side
    if (typeof window === "undefined") return; //Works only on browser with window object

    const initializeWidget = () => {
      // Sub-function to initialize the widget
  ...
  const openWidget = () => {
    // Function to open up the widget
